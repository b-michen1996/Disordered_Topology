...
def H_final_V3(kx, ky, r, epsilon_1, epsilon_2, gamma, gamma_2):
	...
	h_x = gamma * np.sin(kx)
	h_y = (damp) * np.sin(ky) 
	h_z = gamma_2 * (r - np.cos(2 * kx)) - (damp) * np.cos(ky) 
	
	#h_x = np.sin(kx)
	#h_y = np.sin(ky) 
	#h_z = r - np.cos(ky) - np.cos(kx)
...
def Berry_curvature_integral_final_V3(N, r, epsilon_1, epsilon_2, gamma, gamma_2 = 1):
	"""Integral of Berry curvature up to Fermi energy"""
	H_bloch = lambda kx, ky: H_final_V3(kx, ky, r, epsilon_1, epsilon_2, gamma, gamma_2)
	Phi_vals, C_n, k_x_vals, k_y_vals, energies = Berry_curvature(N, H_bloch)
