...
def get_cond_final_wraparound(Nx, Ny, epsilon_1, epsilon_2, gamma, gamma_2, W, E_vals, n_vec_KPM = 10, n_moments_KPM = 100, sigma_entries = [1]):
...
def export_Kubo_contour_final(W_min, W_max, E_min, E_max, n_W, n_E, n_inst, Nx, Ny, epsilon_1, epsilon_2, gamma, gamma_2, run_nr, n_vec_KPM = 10, n_moments_KPM = 100, sigma_entries = [0,1,2,3]):
	"""Compute conductance for disorder realizations of the double cone CI model frome Kubo formula and export"""
	# create folder
	foldername = "Kubo_final_results/Kubo_final_run_" + str(run_nr)

	print(f"Starting Kubo_final run {run_nr}")
	...
	for j_W in range(0, n_W):
		...
		for j_inst in range(0, n_inst):			
			# get conductance			
			cond = get_cond_final_wraparound(Nx, Ny, epsilon_1, epsilon_2, gamma, gamma_2, W_curr, E_vals, n_vec_KPM, n_moments_KPM, sigma_entries)					
...
def main():
	...
	export_Kubo_contour_final(W_min, W_max, E_min, E_max, n_W, n_E, n_inst, Nx, Ny, epsilon_1, epsilon_2, gamma, gamma_2, run_nr, n_vec_KPM, n_moments_KPM, sigma_entries)
